...
let handler = async (m, { text, usedPrefix, command }) => {
    if (!text) throw `*Example:* ${usedPrefix + command} vynaavalerie`   
    try {     
        let json = await fetch(`https://api.vtech.biz.id/api/stalk/github?username=${text}&apikey=${vtech}`).then(res => res.json());
