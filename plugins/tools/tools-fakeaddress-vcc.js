...
let handler = async (m, {
  ...
  if (command == 'vccgenerator' || command == 'vccgen') {
    ...
    m.reply(wait);
    try {
      let vcc = await fetch(`https://api.vtech.biz.id/api/tools/vccgen?jumlah=${jumlah}&apikey=${vtech}`).then(res => res.json());
  ...
  if (command == 'fakeaddress' || command == 'addressgenerator') {
    ...
    try {
      if (!text) {
        let list_country = await fetch(`https://api.vtech.biz.id/api/tools/random-address?country=&apikey=${vtech}`).then(res => res.json());
        ...
        let address = await fetch(`https://api.vtech.biz.id/api/tools/random-address?country=${text}&apikey=${vtech}`).then(res => res.json());
