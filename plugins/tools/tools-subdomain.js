...
async function whois(subdomain, domain) {
  const url = `https://api.vtech.biz.id/api/tools/whois-subdo?subdomain=${encodeURIComponent(subdomain)}&domain=${encodeURIComponent(domain)}&apikey=${vtech}`;
  try {
...
async function create(subdomain, domain, type, content, proxied) {
  const url = `https://api.vtech.biz.id/api/tools/create-subdo?subdomain=${encodeURIComponent(subdomain)}&domain=${encodeURIComponent(domain)}&type=${encodeURIComponent(type)}&content=${encodeURIComponent(content)}&proxied=${proxied}&apikey=${vtech}`;
  try {
    const response = await fetch(url, { method: 'GET' });
    return await response.json();
  } catch (error) {
