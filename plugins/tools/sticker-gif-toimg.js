...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (/webp/.test(mime)) {
        ...
        try {
            ...
            let json;
            if (command === 'togif') {        
                json = await (await fetch(`https://api.vtech.biz.id/api/tools/webp2mp4?url=${media}&apikey=${vtech}`)).json();
            } else if (command === 'toimg') {
                json = await (await fetch(`https://api.vtech.biz.id/api/tools/webp2png?url=${media}&apikey=${vtech}`)).json();
            }
            if (json && json.result) {
