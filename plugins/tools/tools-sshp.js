...
var handler = async (m, { conn, command, args }) => {
  ...
  try {
    var img = await fetch(`https://api.vtech.biz.id/api/tools/sshp?url=${url}&apikey=${vtech}`);
    if (!img) {
      await m.reply('Gagal saat percobaan pertama. Memulai percobaan kedua...');
      img = await fetch(`https://api.vtech.biz.id/api/tools/sshp?url=${url}&apikey=${vtech}`);
