...
async function generateMusic(title, style, lyrics) {
    ...
    const { data } = await axios.post('https://api.vtech.biz.id/api/maker/aimusic', payload);

    let status = 'pending';
    let result = null;
    while (status === 'pending') {
        await new Promise(r => setTimeout(r, 10000));
        const { data: statusData } = await axios.get('https://api.vtech.biz.id/api/maker/aimusic/status', {
