...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  try {
    if (/image/.test(mime) && !/webp/.test(mime)) {
      ...
      baseUrl = `https://api.vtech.biz.id/api/search/bard-img?url=${media}&text=${text}&apikey=${vtech}`;
    } 
    else if (/video/.test(mime)) {
      if (q.seconds > 60) throw 'Maximum video duration is 60 seconds!';
      let buffer = await q.download();
      media = await uploadFile(buffer);
      baseUrl = `https://api.vtech.biz.id/api/search/bard-video?url=${media}&text=${text}&apikey=${vtech}`;
    }
    else if (/audio/.test(mime)) {
      let buffer = await q.download();
      media = await uploadFile(buffer);
      baseUrl = `https://api.vtech.biz.id/api/search/bard-audio?url=${media}&text=${text}&apikey=${vtech}`;
    }
    else {
