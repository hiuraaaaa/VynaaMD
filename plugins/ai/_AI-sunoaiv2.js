...
async function generateMusic(prompt, duration) {
    ...
    const { data } = await axios.post('https://api.vtech.biz.id/api/maker/aimusicv2', payload);
    ...
    while (status === 'pending') {
        ...
        const { data: statusData } = await axios.get('https://api.vtech.biz.id/api/maker/aimusicv2/status', {
