...
const handler = async (m, { conn, args, text, usedPrefix, command }) => {
    ...
    let res;
    if (command === 'brat') {
        res = `https://api.vtech.biz.id/api/maker/brat?text=${encodeURIComponent(text.substring(0, 151))}&apikey=${vtech}`;
    } else if (command === 'brat2' || command === 'bratgif') {
        res = `https://api.vtech.biz.id/api/maker/brat-video?text=${encodeURIComponent(text.substring(0, 151))}&apikey=${vtech}`;
    } else if (command === 'bratvid') {
        res = `https://api.vtech.biz.id/api/maker/brat-video?text=${encodeURIComponent(text.substring(0, 151))}&apikey=${vtech}`;
    } else if (command === 'ttp') {
        res = `https://api.vtech.biz.id/api/maker/ttp?text=${encodeURIComponent(text.substring(0, 151))}&apikey=${vtech}`;
    } else if (command === 'attp') {
        res = `https://api.vtech.biz.id/api/maker/attp?text=${encodeURIComponent(text.substring(0, 151))}&apikey=${vtech}`;
    }
